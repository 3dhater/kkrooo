...
kkScene3DObject* g_pickedObject1 = nullptr;
kkScene3DObject* g_pickedObject2 = nullptr;
...
void pick_2_vertex_weld(s32 id, void* data)
{
	...
	g_pickedObject2 = app->getPickedObject();
	app->setDrawPickLine(false);

	if(g_pickedVertex1 && g_pickedVertex2)
	{
		if(g_pickedObject2 == g_pickedObject1)
		{
			auto object = (Scene3DObject*)g_pickedObject1;
...
void pick_1_vertex(s32 id, void* data)
{
	...
	g_pickedObject1 = app->getPickedObject();
...
void select_edge_loop(s32 id, void* data)
{
	auto app = kkSingleton<Application>::s_instance;
	Scene3D* scene = *app->getScene3D();
	auto object = GetSelectedObject();
...
void Application::_initEditParamsWindow()
{
	...
	m_edit_params_window->BeginGroup(u"Selection", true);
	m_edit_params_window->AddNewLine(0.f, kkPluginGUIParameterType::Polygon);
	m_edit_params_window->AddMoveLeftRight(20.f, kkPluginGUIParameterType::Polygon);
	m_edit_params_window->AddButton(u"+", v2f(20.f, 20.f), select_add_polygon,0, kkPluginGUIParameterType::Polygon);
	m_edit_params_window->EndGroup();
}