...
struct kkPolygon
{
	...
	enum _flags
	{
		...
		// те полигоны, которые будут потом удалены\сохранены
		EF_OLD = 2, // при создании модели они игнорируются
		EF_NEW = 4, // эти полигоны должны быть заменой для old
		
		// чтобы понять, какие полигоны нужно отредактировать
		// (чтобы не выделять память для массивов собирая нужные полигоны)
		// потом после редактирования нужно убрать
		EF_MODIF = 8, 
	};
	u8 m_flags = 0;
};
